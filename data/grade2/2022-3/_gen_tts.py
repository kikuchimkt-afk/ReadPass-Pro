# -*- coding: utf-8 -*-
"""Generate and verify the 70 learning-audio files for Grade 2 2022-3."""
...
def needs_audio(path, relative_path, expected_signature, old_manifest):
    if not os.path.exists(path) or os.path.getsize(path) < 500:
        return True
    entry = old_manifest.get(relative_path)
    if not isinstance(entry, dict):
        return True
    return (
        entry.get("inputSha256") != expected_signature
        or entry.get("audioSha256") != file_sha256(path)
    )
...
for index, focus_point in enumerate(focus_points, 1):
    ...
    english = re.sub(r"\[Source:.*?\]\s*", "", passage.get("en", ""), count=1).strip()
...
for index, item in enumerate(data.get("vocabulary", []), 1):
    slug = "_".join(filter(None, re.sub(
        r"[^a-zA-Z0-9]+", " ", item["word"].lower()
    ).split()))
...
with open(manifest_temporary_path, "w", encoding="utf-8", newline="\n") as handle:
