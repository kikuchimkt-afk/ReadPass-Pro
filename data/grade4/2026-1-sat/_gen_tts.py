# -*- coding: utf-8 -*-
"""Generate TTS audio for vocabulary words and examples - 2026-1-sat grade4"""
import asyncio
import json
import os
import re
...
for sec in d.get("sections", []):
    for q in iter_section_questions(sec):
        ...
        if needs_gen(out):
            asyncio.run(gen_tts(question_tts_text(q, sec.get("type")), out))
            q_count += 1
...
print(f"\nDone: {q_count} new question / {word_count} new word / {ex_count} new example / {lp_count} lessonPlan")
